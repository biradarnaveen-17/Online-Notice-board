...
# NOTE: Adjust path if your database.db is inside 'instance/'
db_path = 'database.db'
...
try:
    ...
    # 3. Create the SystemState table
    try:
        cursor.execute("""
    ...
    # 4. Add the 'reset_token' column to User (for password reset)
    try:
        cursor.execute("ALTER TABLE user ADD COLUMN reset_token VARCHAR(100)")
        print("✅ Added 'reset_token' column to User.")
    except sqlite3.OperationalError:
        print("ℹ️ 'reset_token' column already exists on User.")


    conn.commit()
    conn.close()
    print("\nSUCCESS: Database patched. You can now run app.py")
